...
import Type (Existential, Type(..), everywhereOnType, occurs, substituteUniversal)

data ContextElement
    = CUniversal String
    | CExistential Existential (Maybe Type)
    | CDeclaration String Type
    | CMarker Existential

type Context = Array ContextElement

--------- Helpers
getVariable :: String -> Context -> Maybe Type
getVariable target = findMap case _ of
    CDeclaration name ty | name == target -> Just ty
    _ -> Nothing

solve :: Existential -> Type -> Context -> Context
...
getSolution :: Context -> Existential -> Maybe Type
...
makeExistential :: forall r. String -> Run ( supply :: SUPPLY Int | r ) Existential
makeExistential name = generate <#> { name, id: _ }

--------- Type checking & friends
-- | Check a type is at least as general as another.
subtype :: forall r. Context -> Type -> Type -> Run ( supply :: SUPPLY Int | r ) Context
subtype ctx = flip on (applyContext ctx) case _, _ of
    -- forall-left
    Forall name left, right -> do
...
instantiate :: forall r. InstantiationRule -> Context -> Existential -> Type -> Run ( supply :: SUPPLY Int | r ) Context
...
inferCall :: forall r. Context -> Type -> Expr -> Run ( supply :: SUPPLY Int | r ) (Tuple Context Type)
...
infer :: forall r. Context -> Expr -> Run ( supply :: SUPPLY Int | r ) (Tuple Context Type)
infer ctx = zonk case _ of
    ...
    ExprLambda name body -> do
        ...
        let ctx' = ctx <> [ CExistential exLeft Nothing
                          , CExistential exRight Nothing
                          , declaration ]
...
check :: forall r. Context -> Expr -> Type -> Run ( supply :: SUPPLY Int | r ) Context
...
infer_ :: Expr -> Tuple Context Type
